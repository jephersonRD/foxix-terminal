...
impl Layout for VerticalLayout {
    ...
    fn apply(&self, windows: &mut [Window], container_width: f32, container_height: f32) {
        ...
        let first_width = container_width * self.bias;
        let second_width = container_width - first_width;

        windows[0].x = 0.0;
        windows[0].y = 0.0;
        windows[0].width = first_width;
        windows[0].height = container_height;
        windows[0].is_visible = true;

        for (i, window) in windows.iter_mut().enumerate().skip(1) {
            window.x = first_width + (i - 1) as f32 * second_width / (n - 1) as f32;
            window.y = 0.0;
            window.width = second_width / (n - 1) as f32;
